...
# Normalizar columnas sin importar la versión del CSV
if 'nombre' in estudiantes.columns:
    estudiantes = estudiantes.rename(columns={'nombre': 'nombre_estudiante'})
if 'nombre' in materias.columns:
    materias = materias.rename(columns={'nombre': 'nombre_materia'})
if 'creditos' not in materias.columns:
    materias['creditos'] = 0
if 'descripcion' not in materias.columns:
    materias['descripcion'] = ''

df = (notas
      .merge(estudiantes[['id_estudiante', 'nombre_estudiante', 'email']],
             on='id_estudiante', how='left')
      .merge(materias[['id_materia', 'nombre_materia', 'descripcion']],
             on='id_materia', how='left'))

print("✅ Datos cargados y unidos")
